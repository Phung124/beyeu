    ...
    while ( $data = mysqli_fetch_array($result) ) 
            ...
            <div class="row">
                <div class="col-3" style="max-width: 100%;">
                    <div class="shadow p-3 mb-5 bg-white rounded">
                        ...
                        <form action="xuliuploadanh.php" method="POST" enctype="multipart/form-data"
                            class="form-horizontal">
                            <div class="col-2">
                                <input type="file" name="fileupload" id="fileupload">
                                <button type="submit" name="submit" class="btn btn-outline-primary btn-sm"
                                    style=" height: 29px; width: 54px;">Đăng</button>
                            </div>
                            </from>
        ...
        ?>
            ...
            <head>
                ...
                <link href="public/styles/mdi-font/css/material-design-iconic-font.min.css" rel="stylesheet"
                    media="all">
                ...
                <link href="public/styles/bootstrap-progressbar/bootstrap-progressbar-3.3.4.min.css" rel="stylesheet"
                    media="all">
...
if($result->num_rows > 0)
{
                ...
                { 
                    ...
                    <img src="<?php echo $data['Anh'] ;?>" class="rounded-circle" class="rounded-lg"
                        class="img-fluid rounded-circle" style="width:50px; height:50px;margin-bottom: 50px;">
                ...
                ?>
                    <img src="<?php echo $data['Anh'] ;?>" class="rounded-circle" class="rounded-lg"
                        class="img-fluid rounded-circle" style="width:50px; height:50px;margin-bottom: 50px;"><strong>
                        ...
                        $("#submit<?php echo $data['id_post'];?>").click(function(event) {
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    if (data.error != '') {
                                        console.log(data);
                                        $('#idlike<?php echo $data['id_post'];?>').val(
                                            '<?php echo $data['id_post'];?>');
                                        load_like();
                                    }
                                }
                ...
                if($result2->num_rows > 0)
                    {
                        ...
                        <div id="comment-<?php echo $data2["id"];?>">
                            ...
                            <div class="p-2 mb-2 bg-light text-dark">
                                <img src="<?php echo $data2['Anh'] ;?>" class="rounded-circle" class="rounded-lg"
                                ...
                                <div class="container">
                                    <div class="row justify-content-start">
                                        ...
                                        <input type="submit" name="delete" id="delete<?php echo $data2['id'];?>"
                                ...
                                <h3>
                                    <div class="message-content"><?php echo $data2['noidung_comment']; ?></div>
                                </h3>
                        ...
                        function updatecomment(id) {
                            ...
                            var editMarkUp = '<textarea rows="5" cols="80" id="txtmessage">' + currentMessage +
                    ...
                    <form method="POST" id="comment_form<?php echo $data['id_post']; ?>">
                        ...
                        $(document).ready(function() {
                            $('#sub<?php echo $data['id_post'];?>').click(function(event) {
                                event.preventDefault();
                                var form_data = $('#comment_form<?php echo $data['id_post'] ;?>')
                                    .serialize();
                                $.ajax({
                            ...
                            function load_comment() {
                                ...
                                $.ajax({
                                    ...
                                    data: id,
                                    success: function(data) {
                                        $('#display_comment<?php echo $data['id_post']; ?>').html(
                                            data);
...
if ($result->num_rows > 0) {
  ...
  exit();
             ...
             $sql = "SELECT tenbe,ngaysinhbe,gioitinhbe FROM be WHERE username='$user'";
              $result = $mysqli->query($sql); 
              if ($result->num_rows > 0) 
              { $row = $result->fetch_assoc(); 
              $gender = $row['gioitinhbe'];
               ...
               $tenbe = $row['tenbe'];
                ...
                $monthss = $interval->y * 12 + $interval->m;
                 ...
                 { // Nếu không tìm thấy thông tin người dùng, điều hướng đến trang đăng nhập hoặc trang lỗi exit(); 
                  } 
                 $sql = "SELECT tenme,ngaydusinh FROM me WHERE username='$user'"; 
                 $result = $mysqli->query($sql); 
                 if ($result->num_rows > 0)
                  { $row = $result->fetch_assoc();
                   $ngaydusinh = $row['ngaydusinh']; 
                   $tenme = $row['tenme']; 
                   $dueDateTime = new DateTime($ngaydusinh); 
                   $currentDateTime = new DateTime();
                    $interval = $currentDateTime->diff($dueDateTime); 
                    $weeks = floor($interval->days / 7); 
                    $days = $interval->days % 7; }

                     else { // Nếu không tìm thấy thông tin người dùng, điều hướng đến trang đăng nhập hoặc trang lỗi exit(); 
                     } 
                     ?>
                    ...
                    <div>
                        <div id="motherSection" style="display: none;">
                            ...
                            <div class="main-content2">
                                <div class="due-date"> <span>Dự sinh:</span> <span class="date"><?php echo date('d/m/Y', strtotime($ngaydusinh));?></span> </div>
                                <div class="progress-section2">
                                    <div class="progress-bar2"> <span
                                            class="progress-info"><?php echo "$weeks Tuần $days Ngày"; ?></span>
                                    </div>
                                </div>
                                <div class="buttons2"> <button class="btn-image2"><a href="lichtiem.php" style="color: #FFF;text-decoration: none;">Lịch tiêm</a></button> 
                                <button
                                        class="btn-details2"> <a href="theodoi.php?quanly=theodoict&matd=<?php echo $weeks?>"style="color: #FFF;text-decoration: none;"> Chi Tiết </a> </button>  </div>
                            </div>
                        </div> <!-- Header -->
                        <div id="babySection" style="display: none;">
                            <div class="header3">
                                <div class="user-info3"> <img src="congdong/uploadss/unnamed.png" alt="User Avatar"
                                        class="avatar3"> <span class="username3">Xin chào, <?php echo $tenbe ?>!</span>
                                </div>
                                <h1>Con Yêu</h1>
                            </div> <!-- Main Content -->
                            <div class="main-content3">
                                <div class="age-info3"> <span class="age3"> <?php echo $years ?>Năm</span> <span
                                        class="age3"> <?php echo $months ?> Tháng</span> <span class="age3">
                                        <?php echo $days ?> Ngày</span> </div>
                                <div class="g"
                                    style="display: flex; justify-content: space-around; margin-bottom: 20px;"> <span
                                        class="gender3"
                                        style="background-color: #b684cb; color: white; padding: 10px 20px; border-radius: 5px; font-size: 18px;">Giới
                                        tính: <?php echo $gender ?> </span> </div>
                                <div class="info-buttons3">
                                <button class="info-btn3"> <a href="lichtiem.php" style="color: #FFF;text-decoration: none;">Lịch tiêm</a></button> 
                                     <button class="info-btn3"> <a href="cannang.php" style="color: #FFF;text-decoration: none;">Thông tin con yêu</a></button> 
                                <button class="info-btn3"> <a href="theodoi.php?quanly=theodoiemct&months=<?php echo $monthss?>" style="color: #FFF;text-decoration: none;"> 
                                    Con Yêu Theo Tháng Tuổi </a></button>    
                                <button class="info-btn3"> <a href="chiiucao.php?thang=<?php echo $monthss?>&gender=<?php echo $gender ?>" style="color: #FFF;text-decoration: none;"> 
                                    CânNặng & Chiều Cao</button> </a>
