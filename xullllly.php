...
{ 
    session_start(); 
        ...
        $sql2 = "SELECT * FROM member where username= '$_GET[name]'";
        ...
        if($result->num_rows > 0)
        {
            while ( $data2 = $result->fetch_assoc() ) 
                ...
                $user_timkiem =$data2['username'];
                  ...
                  $query = "SELECT fullname,birthday,sex,Anh,email from member where username= '$_GET[name]'";
...
FROM post,member where member.username=post.username and member.username= '$_GET[name]' "; 
